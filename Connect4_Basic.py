    ...
    for column in range(COLUMN_LEN):
        for row in range(ROW_LEN):
            ...
            if BOARD[row][column] == 1:
                ...
                color = YELLOW
            pygame.draw.circle(win, color, (int((column + 0.5) * GRID_SIZE),
                                            int((ROW_LEN - (row - 0.5)) * GRID_SIZE)), RADIUS)
...
def win_check():
    for y in range(ROW_LEN):
        for x in range(COLUMN_LEN):
            ...
            if [1, 1, 1, 1] in (row, column, d1, d2) or [2, 2, 2, 2] in (row, column, d1, d2):
                return True
    return False
